...
def free_bacon(score):
    ...
    cubed_number = str(score ** 3)

    score = 0
    for i in range(len(cubed_number)):
        if i % 2 == 0:
            score += int(cubed_number[i])
        else:
            score -= int(cubed_number[i])
    return abs(score) + 1
    # END PROBLEM 2
...
def is_swap(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    number = str(3 ** (player_score + opponent_score))
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=6):
    ...
    if is_swap(new_score, opponent_score):
        if opponent_score > new_score:
            return 0
        return num_rolls # prevents a disadvantageous swap
    elif new_score - score >= margin:
        return 0
    return num_rolls
    # END PROBLEM 11

def next_swap(score0, score1):
    i = 1
    while not is_swap(score0, score1) and i < 20:
        score0 += i
        i += 1
    if is_swap(score0 + i, score1):
        return i
    return None
